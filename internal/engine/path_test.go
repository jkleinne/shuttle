...
func TestResolveCloudSource_RcloneRemote(t *testing.T) {
	resolved, isRemote, isDir, err := resolveCloudSource("crypt_gdrive:Documents", "/tmp/drive")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !isRemote {
	...
	if resolved != "crypt_gdrive:Documents" {
		t.Errorf("resolved = %q, want crypt_gdrive:Documents", resolved)
	}
}
...
func TestResolveCloudSource_RelativePath(t *testing.T) {
	drive := t.TempDir()
	subdir := filepath.Join(drive, "ebooks")
	os.Mkdir(subdir, 0o755)

	resolved, isRemote, isDir, err := resolveCloudSource("ebooks", drive)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if isRemote {
