...
func checkSymbol(level CheckLevel, colorMode TerminalColorMode) string {
	switch level {
	case CheckFail:
		return colorize(colorMode, ansiRed, symbolFailed)
	case CheckWarn:
		return colorize(colorMode, ansiYellow, symbolWarn)
	default:
...
// colorMode controls Shuttle-owned ANSI styling after external text is clean.
// An empty Checks slice renders header + "no checks run" footer without panicking.
func RenderReport(w io.Writer, report Report, colorMode TerminalColorMode) {
	...
	for _, c := range report.Checks {
		...
		name := SanitizeTerminalText(c.Name)
		detail := SanitizeTerminalText(c.Detail)
		_, _ = fmt.Fprintf(w, "  %s %-*s  %s\n", checkSymbol(c.Level, colorMode), nameWidth, name, detail)
	}
